...
#define DETECT_WINDOW_SIZE       3U
#define DETECT_CONFIRM_COUNT     2U
...
void usb_pcdc_callback(usb_callback_args_t * p_args)
{
    switch (p_args->event)
    {
        ...
        case USB_STATUS_READ_COMPLETE:
        {
            ...
            if (0U == g_usb_rx_size || g_usb_rx_size > USB_RX_CHUNK_SIZE)
            {
...
static void usb_start_read(void)
{
    ...
    R_USB_Read(&RA8_USB_CTRL, g_usb_rx_chunk, USB_RX_CHUNK_SIZE, USB_CLASS_PCDC);
}
...
static void usb_send_line(const char * line)
{
    ...
    R_USB_Write(&RA8_USB_CTRL, (uint8_t *) line, len, USB_CLASS_PCDC);
}
...
static void handle_agent_ready(const char * line)
{
    ...
    if (RA_STATE_WAIT_USB == g_state || RA_STATE_READY == g_state)
    {
        send_start_scan();
        g_state = RA_STATE_SCANNING;
    }
